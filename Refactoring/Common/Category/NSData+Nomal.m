...
+ (NSData *)dataFromBase64String:(NSString *)aString encoding:(NSStringEncoding)encode{
    ...
    NSData * result=[data base64Decodeds];
    return result;
}

- (NSString *)base64EncodedStringEncoding:(NSStringEncoding)encoding{
    
    NSData * data=[self base64Encodeds];
...
- (NSString *)base64DecodedStringEncoding:(NSStringEncoding)encoding{
    ...
    NSData * data=[self base64Decodeds];
    __autoreleasing NSString * result=[[NSString alloc] initWithData:data encoding:encoding];
    return result;
}
...
- (NSData *)dataUsingDESDencryWithkey:(NSData *)keyData withIV:(void *)iv_{
    ...
    return [[self class] cipherData:self withkey:keyData withIV:iv_ Operation:kCCDecrypt Algorithm:kCCAlgorithmDES keySize:kCCKeySizeDES options:kCCOptionECBMode];
}
